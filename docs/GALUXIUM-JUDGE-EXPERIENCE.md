...
The 0.6 public projection is produced by the same production compatibility planner used by the local product. It records local Capability Memory identity and reload, all three compatibility states, a real pre-write incompatible refusal, deterministic Blueprint to `AGENTS.md` export, source-free Data Boundary custody events, and typed source/destination verification evidence. The protected perturbation answer key remains outside the public export.
...
Try Interactive Demo, Technical Documentation, Security / Local-First Design, Download GRAFT, Watch the product demo, and View source on GitHub work publicly. The downloadable judge artifact, public source repository, and 3:20 Galuxium video are complete. Targeted link acceptance found no broken action link.
