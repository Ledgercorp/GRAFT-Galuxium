...
function bank() {
  return `<div class="page-heading"><div><p class="eyebrow">BEHAVIOR WORTH KEEPING</p><h1>Organ bank</h1><p class="subtitle">Capabilities harvested from your projects, with their source evidence.</p></div></div>${activeStrip()}
...
function renderPlan() {
  ...
  <button class="button primary full" data-action="confirm-apply" ${!plan.safety.ok || disabled() ? 'disabled' : ''}>Apply & verify ${icon('arrow')}</button>`;
}
