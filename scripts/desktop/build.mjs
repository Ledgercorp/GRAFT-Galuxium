...
const stage = path.join(repo, '.desktop-build', fixture ? 'fixture-app' : 'app');
...
for (const directory of ['packages', 'fixtures']) fs.cpSync(path.join(repo, directory), path.join(stage, directory), { recursive: true,
  ...
  filter: (file) => !['test', 'node_modules', '.git'].includes(path.basename(file)) && !(file === excludedPackage || file.startsWith(excludedPackage + path.sep)) && (fixture || path.basename(file) !== 'test-fixture.js') });
...
fs.writeFileSync(path.join(stage, 'package.json'), JSON.stringify({ name: fixture ? 'graft-fixture' : 'graft-desktop', productName: fixture ? 'GRAFT Fixture' : 'GRAFT',
...
if (profile) {
  if (fixture) throw new Error('--product-config does not apply to fixture builds');
...
const options = { dir: stage, arch: process.arch, platform: process.platform, outDir: path.join(repo, 'out', fixture ? 'fixture' : 'desktop') };
